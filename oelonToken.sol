...
contract OELON is ERC20Burnable, ERC20Capped, ERC20Pausable, Ownable, ReentrancyGuard {
    ...
    function getTokenHolderInfo(address holder) public view returns (address[] memory, bool) {
        ...
        return (holdersList, hasHolder);
    }

    function removeHolder(address holder) private {
        (, bool found) = getTokenHolderInfo(holder);
        if (!found) {
            return;
        }
    ...
    function transfer(address to, uint256 amount) public override returns (bool) {
        ...
        if (balanceOf(to) > 0 && !holderExists) {
            holders.push(to);
        }
